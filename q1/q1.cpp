...
int main() {
    ...
    vector<NFATuple> ansNFA = evalReg(reg);
    // printNFA(ansNFA);

    vector<DFATuple> ansDFA = convertNFA(ansNFA);
    // printDFA(ansDFA);

    string cur = query;
    while (true) {
        int response = processInputDFA(ansDFA, cur);
        if (response == -1) {
            ofs << "@" << cur[0];
            if (cur.length() == 1) {
                ofs << "#" << endl;
            ...
            ofs << "$" << cur.substr(0, response + 1);
            if (response + 1 == cur.length()) {
                ofs << "#" << endl;
                return 0;
            }
            cur = cur.substr(response + 1, cur.length() - response - 1);
        }
    }
}
